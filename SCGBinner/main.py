...
def arguments():
    """
    SCGBinner: A long-read binning method based on SCG-guided contrastive learning.

    :return: Parsed command-line arguments
    """
    doc = f"""SCGBinner: A long-read binning method based on SCG-guided contrastive learning.
    Version:{ver}"""
    ...
    parser.add_argument('-v','--version',
                        action='version',
                        help='SCGBinner version')

    subparsers = parser.add_subparsers(title='SCGBinner subcommands',
                                       dest='subcmd',
                                       metavar='')
    ...
    CLtraining_subparsers.add_argument('--contig_file', type=str, help=("The original contigs file."))
    ...
    CLtraining_subparsers.add_argument('--remove_single_copy', default=128, type=int, metavar='N',
                        help='remove_single_copy')
    ...
    CLtraining_subparsers.add_argument('--device_number', default=0, type=int,
                                              help='device_number.')
    CLtraining_subparsers.add_argument('--scg_batch_size', default=0, type=int, metavar='N',
...
def main():
    ...
    # logging
    logger = logging.getLogger('SCGBinner\t'+ver)
    ...
    handler = logging.FileHandler(args.output_path+'/SCGBinner.log')
    ...
    if args.subcmd == 'get_result':
        ...
        run_get_final_result(logger, args, num_threads)
